...
Skat.Comm.Room.prototype = {
	join: function (my_jid, password, initial_presence) {
	...
	configure: function (enforced_values, default_values, create_instant_room) {
	...
	send_configuration: function (user_filled_form) {
		// merge the user filled form into the received form
		this.config_form.merge(user_filled_form, true);
		// turn the form into a submit
		Skat.Comm.connection.sendIQ($iq({type: 'set', to: this.room_jid}).c('query', {xmlns: Strophe.NS.MUC + '#owner'})
		.cnode(this.config_form.submit()), function (iq) {
	...
	presence_handler: function (presence) {
